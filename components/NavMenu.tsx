...
const NavMenu = () => {
  ...
  const handleAddDeveloper = async (e: React.MouseEvent) => {
    e.preventDefault();
    await tracker?.addIntentStrength(Scope.Visitor, {
      dev: {
        str: 50,
      },
    });
  ...
  return (
    <ul className="list-reset lg:flex justify-end flex-1 items-center space-x-2 lg:mr-4">
      ...
      <li>
        <a href="#" onClick={handleAddDeveloper} className="inline-block py-2 px-4 text-black no-underline">
          Add Developer
        </a>
      </li>
      <li>
